...
if __name__ == '__main__':
    ...
    suite2.addTests(orderlist)
    suite3 = unittest.TestSuite()
    suite3.addTests(ShareCount)
    # suite4 = unittest.TestSuite ()
    # suite4.addTests ()
    suite99 = unittest.TestSuite()
    suite99.addTests(testTemp)

    suite_all = [suite1,suite2,suite3]
    ...
    with open(report, 'wb') as f:
        runner = HTMLTestRunner(stream=f, verbosity=2, title='OPSHOP_ADMIN_UI_AutoTest_Report', description='自定义场景测试报告')
        runner.run(alltests)
